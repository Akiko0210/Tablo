...
/** One completed sale: when it happened and how much it was worth. */
...
function emptyPoints(start: Date, g: Granularity): SeriesPoint[] {
  switch (g) {
    ...
    case "week": {
      ...
      return days.map((label) => ({ label, revenue: 0, orders: 0 }));
    }
    case "month": {
    ...
    case "year":
      return MONTHS_SHORT.map((label) => ({ label, revenue: 0, orders: 0 }));
  }
}
...
export function buildSeries(
  ...
  for (const p of points) p.revenue = Math.round(p.revenue * 100) / 100;
